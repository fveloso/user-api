...
export async function getRoleByName(name: string, appId: string) {
    const result = await db.select().from(roles).where(and(eq(roles.name, name), eq(roles.applicationId, appId))).limit(1);
